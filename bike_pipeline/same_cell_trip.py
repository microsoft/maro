# usage:
# python same_cell_trip.py ../../ny ../../ny/same_trip ../../ny/full/h3_201306_202001.station.csv > ../../ny/full/same_cell_trip.csv
...
def concat(data: pd.DataFrame, file: str, station_data: pd.DataFrame, drop_cell: list):
    ret = data[['start station id', 'end station id']]
    ...
    same_cell_trip = len(ret[ret['start_cell'] == ret['end_cell']])

    return same_cell_trip, len(ret)
...
if __name__ == "__main__":
    ...
    for src_file in input_file_list:
        ...
        if r is not None:
            st, at = concat(r, output_data_path, station_data, drop_cell)
            print(src_file,",",st,",",at, ",", st/at*100,"%") 
            total_st += st
            total_at += at
    print("total,", total_st,",",total_at, ",", total_st/total_at*100,"%")
