...
An early stopping checker may be registered if one wishes to terminate training when certain performance requirements
are satisfied, possibly before reaching the prescribed number of episodes.   
...
As an example, the exploration for DQN may be carried out with the aid of an ``EpsilonGreddyExplorer``:
