...
from maro.rl import ExperienceManager


class TestUnboundedStore(unittest.TestCase):
    def setUp(self) -> None:
        self.store = ExperienceManager(["a", "b", "c"])
...
class TestFixedSizeStore(unittest.TestCase):
    def test_put_with_rolling_overwrite(self):
        store = ExperienceManager(["a", "b", "c"], capacity=5, overwrite_type="rolling")
    ...
    def test_put_with_random_overwrite(self):
        store = ExperienceManager(["a", "b", "c"], capacity=5, overwrite_type="random")
        indexes = store.put({"a": [1, 2, 3], "b": [4, 5, 6], "c": [7, 8, 9]})
