...
class TestFrame(unittest.TestCase):
    ...
    def test_enable_snapshots(self):
        for backend_name in backends_to_test:
            ...
            self.assertEqual(0, len(frame.snapshots), msg="snapshot length should be 0")
    ...
    def test_reset(self):
        for backend_name in backends_to_test:
            ...
            self.assertListEqual(
                [1, 234], list(frame.static_nodes[0].a1[:]), msg="static node's a1 should be [1, 234] before reset"
            )
            ...
            # after reset
            self.assertListEqual(
                [0, 0], list(frame.static_nodes[0].a1[:]), msg="static node's a1 should be [0, 0] after reset"
            )
