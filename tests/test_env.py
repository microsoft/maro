...
class TestEnv(unittest.TestCase):
    ...
    def test_env_interfaces_with_specified_business_engine_cls(self):
        ...
        for backend_name in backends_to_test:
            ...
            self.assertEqual(
                1, len(attributes), msg=f"dummy node should only contains 1 attribute, got {len(attributes)}"
            )
            ...
            self.assertEqual("int", val_dtype, msg=f"dummy's val attribute should be int type, got {val_dtype}")
            ...
            self.assertListEqual(
                list(range(0, dummy_number)), env.agent_idx_list, msg=f"dummy engine should have {dummy_number} agents"
            )
            ...
            if backend_name == "dynamic":
                ...
                self.assertListEqual(
                    list(vals_after_reset.flatten()), [0] * dummy_number, msg=f"we should have padding values"
                )
    ...
    def test_early_stop(self):
        ...
        for backend_name in backends_to_test:
            ...
            env = Env(
                business_engine_cls=DummyEngine, start_tick=0, durations=max_tick, options={"post_step_early_stop": 6}
