...
from maro.data_lib.ecr import dump_from_config, load_from_folder
from maro.data_lib.ecr.ecr_data_generator import EcrDataGenerator
from maro.data_lib.ecr.ecr_data_dump import EcrDataDumpUtil
from maro.data_lib.ecr.entities import EcrDataCollection, NoisedItem, PortSetting, VesselSetting, Stop, RoutePoint
...
class TestDumpsLoad(unittest.TestCase):
    def test_load_correct(self):
        ...
        stops_meta_path = os.path.join("tests", "data", "ecr", "data_generator" ,"dumps", "ecr.stops.meta.yml")

        output_folder = tempfile.mkdtemp()

        # here we need to use EcrDataDumpUtil manually to compare the result
        dc: EcrDataCollection =  EcrDataGenerator().gen_data(config_path, 20)
        
        dumper = EcrDataDumpUtil(dc)
    ...
    def _compare_order_proportion(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
        self.assertListEqual(list(dc1.order_proportion), list(dc2.order_proportion))
    
    def _compare_misc(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
    ...
    def _compare_routes(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
        routes_1 = dc1.routes
        routes_2 = dc2.routes
    ...
    def _compare_ports(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
    ...
    def _compare_vessels(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
    ...
    def _compare_stops(self, dc1: EcrDataCollection, dc2: EcrDataCollection):
