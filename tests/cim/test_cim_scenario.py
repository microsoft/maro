        ...
        for backend_name in backends_to_test:
            ...
            for i, port in enumerate(ports):
                ...
                for target in port.target_proportions:
                    self.assertEqual(
                        target.noise, port_config["order_distribution"]["targets"][port_names[target.index]]["noise"]
                    )
            ...
            for i in range(self._business_engine._data_cntr.vessel_number):
                ...
                for stop1, stop3 in zip(vessel_stops_1[i], vessel_stops_3[i]):
                    self.assertListEqual(
                        [stop1.index, stop1.port_idx, stop1.vessel_idx], [stop3.index, stop3.port_idx, stop3.vessel_idx]
                    )
