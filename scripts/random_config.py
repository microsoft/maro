...
def generate_config(
    ...
    vehicle_conf = {"class": "VehicleUnit", "config": {"patient": 100, "unit_transport_cost": 1}}
    ...
    for sku_index, sku_name in enumerate(sku_names):
        sku_list.append({"id": sku_index, "name": sku_name})
    ...
    for store in retailer_list:
        ...
        for i in range(sku_num):
            sku_name = f"SKU{i}"
            store_upstream[sku_name] = [
                warehouse_list[random.randint(0, warehouse_num - 1)]["name"],
            ]
    ...
    for warehouse in warehouse_list:
        ...
        for i in range(sku_num):
            sku_name = f"SKU{i}"
            warehouse_upstream[sku_name] = [
                supplier_facilities[random.randint(0, supplier_num) - 1]["name"],
            ]
