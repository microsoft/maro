...
from maro.simulator.frame import Frame, SnapshotList, FrameAttributeType, FrameNodeType, FrameAttributeNotFoundError, \
    SnapshotSliceError
...
class TestFrame(unittest.TestCase):
    def setUp(self):
        ...
        self.frame.register_attribute("m1", FrameAttributeType.INT_MAT, 10, 2, 5)

        self.frame.setup()

        # init value
        self.frame.set_attribute(FrameNodeType.STATIC, 0, "a1", 0, 123)
        self.frame.set_attribute(FrameNodeType.DYNAMIC, 0, "a1", 0, 1234)

        self.snapshots = SnapshotList(self.frame, MAX_TICK)

    def test_get_attribute_correct(self):
        # take a snapshot first
        self.snapshots.insert_snapshot(self.frame, 0)

        v = self.snapshots.get_attribute(0, FrameNodeType.STATIC, 0, "a1", 0)

        self.assertEqual(v, 123)

        v = self.snapshots.get_attribute(0, FrameNodeType.DYNAMIC, 0, "a1", 0)

        self.assertEqual(v, 1234)

    def test_take_snapshot_correct(self):
        self.snapshots.insert_snapshot(self.frame, 0)  # take snapshot for tick 0
        ...
        self.frame.set_attribute(FrameNodeType.STATIC, 0, "a1", 0, 1234)

        v = self.frame.get_attribute(FrameNodeType.STATIC, 0, "a1", 0)

        self.assertEqual(v, 1234)

        v = self.snapshots.get_attribute(1, FrameNodeType.STATIC, 0, "a1", 0)
    ...
    def test_get_attributes_correct(self):
        ...
        self.snapshots.insert_snapshot(self.frame, 1)

        # query 1st slot of a1 at tick (0, 1)
        states = self.snapshots.get_attributes(FrameNodeType.STATIC, [0, 1], [0, ], ["a1", ], [0, ])
    ...
    def test_get_attributes_with_invalid_ticks(self):
        ...
        states = self.snapshots.get_attributes(FrameNodeType.STATIC, [-1, 0, 1], [0, ], ["a1", ], [0, ])
    ...
    def test_get_attribute_with_undefined_attribute(self):
        ...
        with self.assertRaises(FrameAttributeNotFoundError) as ctx:
            self.snapshots.get_attribute(1, FrameNodeType.STATIC, 0, "a3", 0)
    ...
    def test_snapshot_slice(self):
        ...
        states = self.snapshots.get_attributes(FrameNodeType.STATIC, ticks, ids, attrs, slots)
