...
CurrencyType= Enum(
    ...
    # @classmethod
    # def get_currency_by_name(cls, name: str):
    #     for item in CurrencyType.items():
    #         if item.name == name:
    #             return item
    #     return None
...
class Exchanger(Singleton):
    ...
    def __init__(self, data_path: str, init_time: pd._libs.tslibs.timestamps.Timestamp = pd.Timestamp(year=1970, month=1, day=1) ):
        self._data_path = data_path
        self._init_time = init_time

    def exchange_from_by_date(self, from_currency: CurrencyType, to_currency: CurrencyType, amount: float, cur_date: pd._libs.tslibs.timestamps.Timestamp):
        exchanger_key = from_currency.name + ':' + to_currency.name
        if exchanger_key not in self._exchangers_dict:
            self._exchangers_dict[exchanger_key] = CurrencyExchange(self._data_path, from_currency.name, to_currency.name, init_time=self._init_time)
    ...
    def exchange_to_by_date(self, from_currency: CurrencyType, to_currency: CurrencyType, amount: float, cur_date: pd._libs.tslibs.timestamps.Timestamp):
        exchanger_key = from_currency.name + ':' + to_currency.name
        if exchanger_key not in self._exchangers_dict:
            self._exchangers_dict[exchanger_key] = CurrencyExchange(self._data_path, from_currency.name, to_currency.name, init_time=self._init_time)
        exchange_rate = self._exchangers_dict[exchanger_key].get_inter_bank_rate(cur_date)
