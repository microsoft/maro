...
def start_rl_job(parser: ConfigParser, maro_root: str, background: bool = False) -> List[subprocess.Popen]:
    procs = [
        exec(
            f"python {script}",
...
def start_rl_job_with_docker_compose(parser: ConfigParser, context: str, dockerfile_path: str, image_name: str) -> None:
    common_spec = {
    ...
    subprocess.run(["docker-compose", "--project-name", job_name, "up", "--remove-orphans"])
