...
class CitiBikePipeline(DataPipeline):
    ...
    def _read_common_data(self):
        ...
        with open(self._station_info_file, mode="r", encoding="utf-8") as station_file:
            ...
            station_data = raw_station_data.rename(
                columns={"lon": "station_longitude", "lat": "station_latitude", "region_id": "region"}
            )
    ...
    def _process_src_file(self, src_data: pd.DataFrame):
        ...
        trip_data = trip_data.join(stations_existed.set_index("station_id"), on="start_station_id").rename(
            columns={"station_index": "start_station_index"}
        )
        trip_data = trip_data.join(stations_existed.set_index("station_id"), on="end_station_id").rename(
            columns={"station_index": "end_station_index"}
        )
    ...
    def _process_current_topo_station_info(
        ...
        data_station_init = stations_existed.join(
            self._common_data["full_stations"][["station_id", "capacity"]].set_index("station_id"), on="station_id"
        ).join(loc_ref[["station_id", "latitude", "longitude"]].set_index("station_id"), on="station_id")
    ...
    def _process_distance(self, station_info: pd.DataFrame):
        distance_adj = pd.DataFrame(
            0, index=station_info["station_index"], columns=station_info["station_index"], dtype=np.float
        )
...
class CitiBikeToyPipeline(DataPipeline):
    ...
    def _gen_distance(self, station_init: pd.DataFrame):
        ...
        distance_adj = pd.DataFrame(
            0, index=station_init["station_index"], columns=station_init["station_index"], dtype=np.float
        )
        ...
        if os.path.exists(config_path):
            with open(config_path) as fp:
                ...
                self._data_pipeline["weather"] = WeatherToyPipeline(
                    topology=topology, start_time=cfg["start_time"], end_time=cfg["end_time"], is_temp=is_temp
                )
        else:
            ...
            for topology in self._conf["trips"].keys():
                if topology.startswith("toy"):
                    self.topologies[topology] = CitiBikeToyTopology(
                        topology=topology, config_path=self._conf["trips"][topology]["toy_meta_path"], is_temp=is_temp
                    )
                else:
...
class NOAAWeatherPipeline(WeatherPipeline):
    ...
    def _gen_fall_back_file(self):
        fall_back_content = ['"STATION","DATE","AWND","PRCP","SNOW","TMAX","TMIN"\n', ",,,,,,\n"]
