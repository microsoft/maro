...
class LocalResourceExecutor:
    ...
    def add_cluster(self):
        if self._redis_connection.hexists(LocalParams.RESOURCE_INFO, "connections"):
            current_connection = self._redis_connection.hget(LocalParams.RESOURCE_INFO, "connections")
        else:
    ...
    def _quit(self):
        ...
        agent_pid = self._redis_connection.hget(LocalParams.RESOURCE_INFO, "agent_pid")
    ...
    def set_available_resource(self, available_resource: dict):
        self._redis_connection.hset(LocalParams.RESOURCE_INFO, "available_resource", json.dumps(available_resource))

    def get_local_resource(self):
        if self._redis_connection.hexists(LocalParams.RESOURCE_INFO, "resource"):
            static_resource = json.loads(self._redis_connection.hget(LocalParams.RESOURCE_INFO, "resource"))
        else:
    ...
    def get_local_resource_usage(self, previous_length: int):
        usage_dict = {}
        usage_dict["cpu"] = self._redis_connection.lrange(LocalParams.CPU_USAGE, previous_length, -1)
        usage_dict["memory"] = self._redis_connection.lrange(LocalParams.MEMORY_USAGE, previous_length, -1)
        usage_dict["gpu"] = self._redis_connection.lrange(LocalParams.GPU_USAGE, previous_length, -1)
