...
"""
    ...
    os.system(f"cd {os.path.dirname(TERMINAL_PATH)} ;" f"python {TERMINAL_PATH} &")


def stop_admin(*args, **kwargs):

    os.system(f"pkill -f '{TERMINAL_PATH}' ")


def requirements_admin(*args, **kwargs):
    os.system(f"cd {os.path.dirname(TERMINAL_PATH)} ;" f"pip install -r requirements.txt")
