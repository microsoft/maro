...
def create_schedule(**kwargs):
    ...
    redis_controller.set_schedule_details(schedule_name=schedule_details["name"], schedule_details=schedule_details)

    # Build individual jobs
    for job_name in schedule_details["job_names"]:
        redis_controller.set_job_details(
            job_name=job_name, job_details=_build_job_details(schedule_details=schedule_details, job_name=job_name)
        )
