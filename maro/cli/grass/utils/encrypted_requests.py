...
class EncryptedRequests:
    ...
    def __init__(
        ...
        self._master_to_dev_encryption_private_key_obj = serialization.load_pem_private_key(
            master_to_dev_encryption_private_key.encode("utf-8"), password=None
        )
    ...
    def get(self, url: str):
        ...
        # Get response.
        response = requests.get(url=url, headers=self._get_new_headers(aes_key=aes_key, aes_ctr_nonce=aes_ctr_nonce))
    ...
    def post(self, url: str, json_dict: dict = None):
        ...
        response = requests.post(
            url=url,
            headers=self._get_new_headers(aes_key=aes_key, aes_ctr_nonce=aes_ctr_nonce),
            data=None
            if json_dict is None
            else self._get_encrypted_bytes(json_dict=json_dict, aes_key=aes_key, aes_ctr_nonce=aes_ctr_nonce),
        )
    ...
    def _get_decrypted_bytes(self, response: requests.Response) -> bytes:
        ...
        aes_key = self._master_to_dev_encryption_private_key_obj.decrypt(
            ciphertext=base64.b64decode(payload["aes_key"].encode("ascii")), padding=self._get_asymmetric_padding()
        )
            ...
            payload={
                ...
                "aes_key": base64.b64encode(
                    self._dev_to_master_encryption_public_key_obj.encrypt(
                        plaintext=aes_key, padding=self._get_asymmetric_padding()
                    )
                ).decode("ascii"),
                "aes_ctr_nonce": base64.b64encode(
                    self._dev_to_master_encryption_public_key_obj.encrypt(
                        plaintext=aes_ctr_nonce, padding=self._get_asymmetric_padding()
                    )
                ).decode("ascii"),
            },
