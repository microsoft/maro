...
def main():
    ...
    parser_data = subparsers.add_parser(
        "data", help="Data processing tools for MARO binary format.", parents=[global_parser]
    )
    ...
    # maro meta
    parser_meta = subparsers.add_parser("meta", help="Manage the meta files for MARO.", parents=[global_parser])
    ...
    parser_k8s = subparsers.add_parser(
        "k8s", help="Manage distributed cluster with Kubernetes.", parents=[global_parser]
    )
    ...
    parser_aks = subparsers.add_parser(
        "aks", help="Manage distributed cluster with Kubernetes.", parents=[global_parser]
    )
    ...
    parser_inspector = subparsers.add_parser(
        "inspector", help=("Display visualization of experimental data."), parents=[global_parser]
    )
    ...
    # maro local
    parser_local = subparsers.add_parser("local", help="Run jobs locally.")
    ...
    # maro project
    parser_project = subparsers.add_parser("project", help="Manage maro projects.")
    ...
    # maro admin
    parser_admin = subparsers.add_parser("admin", help="Manage maro admin tools.")
...
def load_parser_local(prev_parser: ArgumentParser, global_parser: ArgumentParser) -> None:
    ...
    parser = subparsers.add_parser(
        "run", help="Run a job in debug mode.", examples=CliExamples.MARO_PROCESS_SETUP, parents=[global_parser]
    )
    ...
    parser = subparsers.add_parser(
        "init", help="Initialize local job manager.", examples=CliExamples.MARO_PROCESS_SETUP, parents=[global_parser]
    )
    parser.add_argument(
        "-p",
        "--port",
    ...
    parser = subparsers.add_parser("exit", help="Terminate the local job manager", parents=[global_parser])
    parser.set_defaults(func=exit)

    # maro local job
    parser = subparsers.add_parser("job", help="Manage jobs", parents=[global_parser])
    parser.set_defaults(func=_help_func(parser=parser))
    ...
    job_add_parser = job_subparsers.add_parser(
        "add", help="Start an RL job", examples=CliExamples.MARO_PROCESS_JOB_START, parents=[global_parser]
    )
    ...
    job_stop_parser = job_subparsers.add_parser(
        "rm", help="Stop an RL job", examples=CliExamples.MARO_PROCESS_JOB_STOP, parents=[global_parser]
    )
    ...
    job_list_parser = job_subparsers.add_parser(
        "ls", help="List all jobs", examples=CliExamples.MARO_PROCESS_JOB_LIST, parents=[global_parser]
    )
    ...
    job_logs_parser = job_subparsers.add_parser(
        "logs", help="Get job logs", examples=CliExamples.MARO_PROCESS_JOB_LOGS, parents=[global_parser]
    )
    job_logs_parser.add_argument("job_name", help="job name")
    job_logs_parser.add_argument(
        "-n", "--tail", type=int, default=-1, help="Number of lines to show from the end of the given job's logs"
    )
...
def load_parser_grass(prev_parser: ArgumentParser, global_parser: ArgumentParser) -> None:
    ...
    parser_create = subparsers.add_parser(
        "create", help="Create cluster", examples=CliExamples.MARO_GRASS_CREATE, parents=[global_parser]
    )
    ...
    parser_delete = subparsers.add_parser(
        "delete", help="Delete cluster", examples=CliExamples.MARO_GRASS_DELETE, parents=[global_parser]
    )
    parser_delete.add_argument("cluster_name", help="Name of the cluster")
    parser_delete.set_defaults(func=delete)

    # maro grass node
    parser_node = subparsers.add_parser("node", help="Manage nodes of the cluster", parents=[global_parser])
    ...
    parser_node_start = parser_node_subparsers.add_parser(
        "start", help="Start nodes", examples=CliExamples.MARO_GRASS_NODE_START, parents=[global_parser]
    )
    ...
    parser_node_stop = parser_node_subparsers.add_parser(
        "stop", help="Stop nodes", examples=CliExamples.MARO_GRASS_NODE_STOP, parents=[global_parser]
    )
    ...
    parser_node_list = parser_node_subparsers.add_parser(
        "list", help="List details of nodes", examples=CliExamples.MARO_GRASS_NODE_LIST, parents=[global_parser]
    )
    ...
    parser_node_leave = parser_node_subparsers.add_parser(
        "leave", help="make node leave to cluster", examples=CliExamples.MARO_GRASS_NODE_LEAVE, parents=[global_parser]
    )
    ...
    # maro grass image
    parser_image = subparsers.add_parser("image", help="Manage images of the cluster", parents=[global_parser])
    ...
    # maro grass data
    parser_data = subparsers.add_parser("data", help="Manage user data storage in the cluster", parents=[global_parser])
    parser_data.set_defaults(func=_help_func(parser=parser_data))
    ...
    # maro grass job
    parser_job = subparsers.add_parser("job", help="Manage jobs", parents=[global_parser])
    ...
    parser_job_start = parser_job_subparsers.add_parser(
        "start", help="Start a training job", examples=CliExamples.MARO_GRASS_JOB_START, parents=[global_parser]
    )
    ...
    parser_job_stop = parser_job_subparsers.add_parser(
        "stop", help="Stop a training job", examples=CliExamples.MARO_GRASS_JOB_STOP, parents=[global_parser]
    )
    ...
    parser_job_list = parser_job_subparsers.add_parser(
        "list", help="List details of jobs", examples=CliExamples.MARO_GRASS_JOB_LIST, parents=[global_parser]
    )
    ...
    parser_job_logs = parser_job_subparsers.add_parser(
        "logs", help="Get logs of the job", examples=CliExamples.MARO_GRASS_JOB_LOGS, parents=[global_parser]
    )
    ...
    # maro grass schedule
    parser_schedule = subparsers.add_parser("schedule", help="Manage schedules", parents=[global_parser])
    ...
    parser_schedule_start = parser_schedule_subparsers.add_parser(
        "start", help="Start a schedule", examples=CliExamples.MARO_GRASS_SCHEDULE_START, parents=[global_parser]
    )
    ...
    parser_schedule_stop = parser_schedule_subparsers.add_parser(
        "stop", help="Stop a schedule", examples=CliExamples.MARO_GRASS_SCHEDULE_STOP, parents=[global_parser]
    )
    ...
    parser_clean = subparsers.add_parser(
        "clean", help="Clean cluster", examples=CliExamples.MARO_GRASS_CLEAN, parents=[global_parser]
    )
    ...
    parser_status = subparsers.add_parser(
        "status", help="Get status of the cluster", examples=CliExamples.MARO_GRASS_STATUS, parents=[global_parser]
    )
    ...
    parser_clean = subparsers.add_parser(
        "template", help="Get deployment templates", examples=CliExamples.MARO_GRASS_TEMPLATES, parents=[global_parser]
    )
...
def load_parser_env(prev_parser: ArgumentParser, global_parser) -> None:
    ...
    # maro env list
    parser_list = subparsers.add_parser("list", help="List name of built-in scenarios.", parents=[global_parser])

    parser_list.set_defaults(func=list_scenarios)

    # maro env topologies --scenario name
    parser_topo = subparsers.add_parser(
        "topology", help="Get built-in topologies for specified scenario.", parents=[global_parser]
    )

    parser_topo.add_argument("-s", "--scenario", required=True, help="Scenario name to show topologies.")

    parser_topo.set_defaults(func=list_topologies)

    # MARO env data command
    parser_env_data = subparsers.add_parser(
        "data", help="Generate predefined scenario related data.", parents=[global_parser]
    )
    ...
    generate_cmd_parser = data_subparsers.add_parser(
        "generate", help="Generate data for a specific scenario and topology.", parents=[global_parser]
    )

    generate_cmd_parser.add_argument(
        "-s", "--scenario", required=True, choices=get_scenarios(), help="Scenario of environment."
    )

    generate_cmd_parser.add_argument("-t", "--topology", required=True, help="Topology of scenario.")

    generate_cmd_parser.add_argument("-f", "--forced", action="store_true", help="Re-generate forcibly.")
    generate_cmd_parser.set_defaults(func=generate)

    list_cmd_parser = data_subparsers.add_parser(
        "list", help="List predefined environments that need generate data extraly.", parents=[global_parser]
    )
...
def load_parser_k8s(prev_parser: ArgumentParser, global_parser: ArgumentParser) -> None:
    ...
    parser_create = subparsers.add_parser(
        "create", help="Create cluster", examples=CliExamples.MARO_K8S_CREATE, parents=[global_parser]
    )
    parser_create.add_argument("deployment_path", help="Path of the create deployment")
    parser_create.set_defaults(func=create)
    ...
    parser_create = subparsers.add_parser(
        "delete", help="Delete cluster", examples=CliExamples.MARO_K8S_DELETE, parents=[global_parser]
    )
    ...
    parser_node_scale = parser_node_subparsers.add_parser(
        "list", help="List details of nodes", examples=CliExamples.MARO_K8S_NODE_LIST, parents=[global_parser]
    )
    parser_node_scale.add_argument("cluster_name", help="Name of the cluster")
    ...
    parser_image_push = parser_image_subparsers.add_parser(
        "list", help="List the images in the cluster", examples=CliExamples.MARO_K8S_IMAGE_LIST, parents=[global_parser]
    )
    parser_image_push.add_argument("cluster_name", help="Name of the cluster")
    ...
    parser_data_pull = parser_data_subparsers.add_parser(
        "remove", help="Remove data in the cluster data storage", parents=[global_parser]
    )
    parser_data_pull.add_argument("cluster_name", help="Name of the cluster")
    parser_data_pull.add_argument("remote_path", help="Path of the file in the cluster data storage")
    ...
    parser_job_start = parser_job_subparsers.add_parser(
        "start", help="Start a training job", examples=CliExamples.MARO_K8S_JOB_START, parents=[global_parser]
    )
    parser_job_start.add_argument("cluster_name", help="Name of the cluster")
    parser_job_start.add_argument("deployment_path", help="Path of the job deployment")
    parser_job_start.set_defaults(func=start_job)
    ...
    parser_job_stop = parser_job_subparsers.add_parser(
        "stop", help="Stop a training job", examples=CliExamples.MARO_K8S_JOB_STOP, parents=[global_parser]
    )
    parser_job_stop.add_argument("cluster_name", help="Name of the cluster")
    parser_job_stop.add_argument("job_name", help="Name of the job")
    parser_job_stop.set_defaults(func=stop_job)
    ...
    parser_job_logs = parser_job_subparsers.add_parser("logs", help="Get logs of the job", parents=[global_parser])
    parser_job_logs.add_argument("cluster_name", help="Name of the cluster")
    parser_job_logs.add_argument("job_name", help="Name of the job")
    parser_job_logs.set_defaults(func=get_job_logs)
    ...
    parser_job_list = parser_job_subparsers.add_parser("list", help="List details of jobs", parents=[global_parser])
    parser_job_list.add_argument("cluster_name", help="Name of the cluster")
    parser_job_list.set_defaults(func=list_job)
    ...
    parser_schedule_start = parser_schedule_subparsers.add_parser(
        "start", help="Start a schedule", examples=CliExamples.MARO_K8S_SCHEDULE_START, parents=[global_parser]
    )
    parser_schedule_start.add_argument("cluster_name", help="Name of the cluster")
    parser_schedule_start.add_argument("deployment_path", help="Path of the schedule deployment")
    parser_schedule_start.set_defaults(func=start_schedule)
    ...
    parser_schedule_stop = parser_schedule_subparsers.add_parser(
        "stop", help="Stop a schedule", examples=CliExamples.MARO_K8S_SCHEDULE_STOP, parents=[global_parser]
    )
    parser_schedule_stop.add_argument("cluster_name", help="Name of the cluster")
    parser_schedule_stop.add_argument("schedule_name", help="Name of the schedule")
    parser_schedule_stop.set_defaults(func=stop_schedule)
    ...
    parser_status = subparsers.add_parser(
        "status", help="Get status of the cluster", examples=CliExamples.MARO_K8S_STATUS, parents=[global_parser]
    )
    parser_status.add_argument("cluster_name", help="Name of the cluster")
    parser_status.set_defaults(func=status)
    ...
    parser_template = subparsers.add_parser(
        "template", help="Get deployment templates", examples=CliExamples.MARO_K8S_TEMPLATE, parents=[global_parser]
    )
...
def load_parser_aks(prev_parser: ArgumentParser, global_parser: ArgumentParser) -> None:
    ...
    parser_create = subparsers.add_parser(
        "exit", help="Delete deployed resources", examples=CliExamples.MARO_K8S_DELETE, parents=[global_parser]
    )
    parser_create.set_defaults(func=exit)

    # maro aks job
    parser_job = subparsers.add_parser("job", help="AKS job-related commands", parents=[global_parser])
    parser_job.set_defaults(func=_help_func(parser=parser_job))
    ...
    parser_job_start = job_subparsers.add_parser(
        "add", help="Add an RL job to the AKS cluster", examples=CliExamples.MARO_K8S_JOB_START, parents=[global_parser]
    )
...
def load_parser_data(prev_parser: ArgumentParser, global_parser: ArgumentParser):
    ...
    build_cmd_parser.add_argument("--meta", type=str, required=True, help="Metafile for binary file building.")
    ...
    build_cmd_parser.add_argument(
        "--output", type=str, required=True, help="Path (with file name) to dump the binary file."
    )
...
def load_parser_meta(prev_parser: ArgumentParser, global_parser: ArgumentParser):
    ...
    deploy_cmd_parser = meta_cmd_sub_parsers.add_parser(
        "deploy", help="Deploy data files for MARO.", parents=[global_parser]
    )
...
def load_parser_inspector(prev_parser: ArgumentParser, global_parser: ArgumentParser):
    ...
    dashboard_cmd_parser = inspector_cmd_sub_parsers.add_parser(
        "dashboard", fromfile_prefix_chars="@", help="Dashboard of selected env displayed.", parents=[global_parser]
    )
    ...
    geo_cmd_parser = inspector_cmd_sub_parsers.add_parser(
        "geo", fromfile_prefix_chars="@", help="Geographic data display.", parents=[global_parser]
    )

    geo_cmd_parser.add_argument(
        "--start", type=str, help="Kind of container expected to start, Database or Service.", required=True
    )

    geo_cmd_parser.add_argument(
        "--experiment_name", type=str, required=False, help="Name of experiment expected to be displayed."
    )

    geo_cmd_parser.add_argument("--front_end_port", type=int, required=False, help="Specified port of front_end.")
...
def load_parser_project(prev_parser: ArgumentParser, global_parser: ArgumentParser):
    ...
    new_cmd_parser = sub_parsers.add_parser(
        "new", help="Generate a new project under current directory to work with MARO.", parents=[global_parser]
    )
...
def load_parser_admin(prev_parser: ArgumentParser, global_parser: ArgumentParser):
    ...
    start_parser = sub_parsers.add_parser("start", help="Start MARO admin web server.", parents=[global_parser])
    ...
    stop_parser = sub_parsers.add_parser("stop", help="Stop MARO admin web server.", parents=[global_parser])
    ...
    req_parser = sub_parsers.add_parser(
        "req", help="Install requirements for MARO admin web server.", parents=[global_parser]
    )
