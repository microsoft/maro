...
def get_new_port_number(experiment_name: str) -> json:
    ...
    """
    params = {"query": f"select count(episode) from {experiment_name}.port_details", "count": "true"}
...
def process_port_data(db_port_data: json) -> json:
    ...
    original_port_data["port_name"] = list(
        map(lambda x: port_json_data[int(x)]["tooltip"], original_port_data["index"])
    )
    original_port_data["position"] = list(
        map(lambda x: port_json_data[int(x)]["position"], original_port_data["index"])
    )
