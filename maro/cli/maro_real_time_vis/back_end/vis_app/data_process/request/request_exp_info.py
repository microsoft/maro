...
def get_experiments():
    ...
    exps = requests.get(
        url=request_settings.request_url.value, headers=request_settings.request_header.value, params=get_exps_params
    ).json()
...
def get_experiment_info():
    ...
    params = {"query": f"select * from maro.experiments where name='{exp_name}'", "count": "true"}
    ...
    min_episode = requests.get(
        url=request_settings.request_url.value, headers=request_settings.request_header.value, params=episode_params
    ).json()
    ...
    total_episode = requests.get(
        url=request_settings.request_url.value, headers=request_settings.request_header.value, params=total_params
    ).json()
    ...
    port_number = requests.get(
        url=request_settings.request_url.value, headers=request_settings.request_header.value, params=port_number_params
    ).json()
...
def add_pending_experiment(experiment_name):
    ...
    requests.get(
        url=request_settings.request_url.value, headers=request_settings.request_header.value, params=next_exp_params
    ).json()
