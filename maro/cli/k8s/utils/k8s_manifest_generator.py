...
def get_azurefile_volume_spec(name: str, share_name: str, secret_name: str):
    return {"name": name, "azureFile": {"secretName": secret_name, "shareName": share_name, "readOnly": False}}
...
def get_container_spec(image_name: str, component_name: str, script: str, env: dict, volumes):
    ...
    return {
        **common_container_spec,
        **{"name": component_name, "command": ["python3", script], "env": format_env_vars(env, mode="k8s")},
    }


def get_redis_deployment_manifest(host: str, port: int):
    return {
        "metadata": {"name": host, "labels": {"app": "redis"}},
        "spec": {
            "selector": {"matchLabels": {"app": "redis"}},
            "replicas": 1,
            "template": {
                "metadata": {"labels": {"app": "redis"}},
                "spec": {"containers": [{"name": "master", "image": "redis:6", "ports": [{"containerPort": port}]}]},
            },
        },
    }
...
def get_redis_service_manifest(host: str, port: int):
    return {
        "metadata": {"name": host, "labels": {"app": "redis"}},
        "spec": {"ports": [{"port": port, "targetPort": port}], "selector": {"app": "redis"}},
    }
