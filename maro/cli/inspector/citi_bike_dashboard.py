...
def render_inter_view(source_path: str, epoch_num: int):
    ...
    # Generate data.
    data_summary = helper.read_detail_csv(os.path.join(source_path, GlobalFileNames.stations_sum)).iloc[
        down_pooling_range
    ]
...
def render_intra_view(source_path: str, epoch_num: int, prefix: str):
    ...
    view_option = st.sidebar.selectbox(label="By station/snapshot:", options=CitiBikeIntraViewChoice._member_names_)
...
def render_top_k_summary(source_path: str, prefix: str, epoch_index: int):
    ...
    data = helper.read_detail_csv(os.path.join(source_path, f"{prefix}{epoch_index}", GlobalFileNames.stations_sum))
    # Convert index to station name.
    index_name_conversion = helper.read_detail_csv(os.path.join(source_path, GlobalFileNames.name_convert))
    data["station name"] = list(map(lambda x: index_name_conversion.loc[int(x[9:])][0], data["name"]))
    # Generate top summary.
    top_number = st.select_slider("Select Top K", list(range(1, 6)))
...
def _generate_intra_view_by_snapshot(
    ...
    selected_station_sample_ratio = st.sidebar.select_slider(
        label="Station Sampling Ratio", options=sample_ratio, value=1
    )
    ...
    if selected_station_sample_ratio == 0:
        ...
        data_filtered["Station Name"] = data_filtered["name"].apply(lambda x: index_name_conversion.loc[int(x)])
    data_melt = data_filtered.melt(["Station Name", "name"], var_name="Attributes", value_name="Count")
...
def _generate_intra_view_by_station(
    ...
    Args:
        ...
        snapshot_num (int): Number of snapshots.
    """
    selected_station = st.sidebar.select_slider(label="station index", options=stations_index)
    ...
    selected_snapshot_sample_ratio = st.sidebar.select_slider(
        label="Snapshot Sampling Ratio:", options=snapshot_sample_ratio_list, value=1
    )
    # Get formula input and output.
    data_formula = helper.get_filtered_formula_and_data(
        GlobalScenarios.CITI_BIKE, data_filtered, attribute_option_candidates
