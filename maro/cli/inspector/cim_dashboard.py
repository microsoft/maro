...
def start_cim_dashboard(source_path: str, epoch_num: int, prefix: str):
    ...
    """
    option = st.sidebar.selectbox(label="Data Type", options=PanelViewChoice._member_names_)
...
def render_inter_view(source_path: str, epoch_num: int):
    ...
    data["remaining_space"] = list(map(lambda x, y, z: x - y - z, data["capacity"], data["full"], data["empty"]))
...
def render_intra_view(source_path: str, epoch_num: int, prefix: str):
    ...
    Args:
        ...
        prefix (str):  Prefix of data folders.
    """
    selected_epoch = st.sidebar.select_slider(label="Choose an Epoch:", options=list(range(0, epoch_num)))
    ...
    st.sidebar.markdown("***")
    option_view = st.sidebar.selectbox(label="By ports/snapshot:", options=CIMIntraViewChoice._member_names_)

    if option_view == CIMIntraViewChoice.by_port.name:
        _render_intra_view_by_ports(
            data_ports, ports_index, index_name_conversion, attribute_option_candidates, snapshot_num
        )
...
def _generate_inter_view_panel(data: pd.DataFrame, down_pooling_range: List[float]):
    ...
    inter_line_chart = (
        alt.Chart(data_melt)
        .mark_line()
        .encode(x="Epoch Index", y="Count", color="Attributes", tooltip=["Attributes", "Count", "Epoch Index"])
    ...
    inter_bar_chart = (
        alt.Chart(data_melt)
        .mark_bar()
        .encode(x="Epoch Index:N", y="Count:Q", color="Attributes:N", tooltip=["Attributes", "Count", "Epoch Index"])
        .properties(width=700, height=380)
    )
...
def _render_intra_view_by_ports(
    ...
    """
    selected_port = st.sidebar.select_slider(label="Choose a Port:", options=ports_index)
    sample_ratio = helper.get_sample_ratio_selection_list(snapshot_num)
    selected_snapshot_sample_ratio = st.sidebar.select_slider(
        label="Snapshot Sampling Ratio:", options=sample_ratio, value=1
    )
    # Accumulated data.
...
def _render_intra_view_by_snapshot(
    ...
    Args:
        ...
        prefix (str): Prefix of data folders.
    """
    selected_snapshot = st.sidebar.select_slider(label="snapshot index", options=snapshots_index)
    # Get sample ratio.
    sample_ratio = helper.get_sample_ratio_selection_list(ports_num)
    selected_port_sample_ratio = st.sidebar.select_slider(label="Ports Sampling Ratio:", options=sample_ratio, value=1)
    # Accumulated data.
...
def _generate_intra_panel_by_ports(
    ...
    data_melt = data_filtered.melt("snapshot_index", var_name="Attributes", value_name="Count")
    port_line_chart = (
        alt.Chart(data_melt)
        .mark_line()
        .encode(
...
def _generate_intra_panel_accumulated_by_snapshot(
    ...
    for index in down_pooling_sample_list:
        data_rename = pd.concat([data_rename, snapshot_filtered[snapshot_filtered["name"] == f"ports_{index}"]], axis=0)
    ...
    data_melt = data_rename.melt(["name", "Port Name"], var_name="Attributes", value_name="Count")
    intra_bar_chart = (
        alt.Chart(data_melt)
        .mark_bar()
        .encode(
...
def _render_intra_panel_vessel(source_path: str, prefix: str, option_epoch: int, snapshot_index: int):
    ...
    """
    data_vessel = helper.read_detail_csv(os.path.join(source_path, f"{prefix}{option_epoch}", "vessels.csv"))
...
def _generate_intra_panel_vessel(data_vessel: pd.DataFrame, snapshot_index: int, vessels_num: int):
    ...
    selected_vessel_sample_ratio = st.sidebar.select_slider(
        label="Vessels Sampling Ratio:", options=sample_ratio, value=1
    )
    ...
    for index in down_pooling_sample_list:
        data_rename = pd.concat([data_rename, data_vessel[data_vessel["name"] == f"vessels_{index}"]], axis=0)
    data_filtered = data_rename.reset_index(drop=True)
    data_filtered["name"] = data_filtered["name"].apply(lambda x: int(x[8:]))
    data_melt = data_filtered.melt("name", var_name="Attributes", value_name="Count")
...
def _generate_intra_heat_map(matrix_data: str):
    ...
    data_transfer_volume = pd.DataFrame(
        {"Dest_Port": np.array(x_axis).ravel(), "Start_Port": np.array(y_axis).ravel(), "Count": np.array(b).ravel()}
    )
    transfer_volume_heat_map = (
        alt.Chart(data_transfer_volume)
        .mark_rect()
        .encode(x="Dest_Port:O", y="Start_Port:O", color="Count:Q", tooltip=["Dest_Port", "Start_Port", "Count"])
        .properties(width=700, height=380)
    )
...
def _generate_top_k_summary(data: pd.DataFrame, snapshot_index: int, index_name_conversion: pd.DataFrame):
    ...
    data_summary["fulfillment_ratio"] = list(
        map(lambda x, y: round(x / (y + 1 / 1000), 4), data_summary["acc_fulfillment"], data_summary["acc_booking"])
    )

    data_summary["port name"] = list(map(lambda x: index_name_conversion.loc[int(x[6:])][0], data_summary["name"]))
    helper.render_h3_title("Select Top k:")
    selected_top_number = st.select_slider(label="", options=list(range(1, 6)))
