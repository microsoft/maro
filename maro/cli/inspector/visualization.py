...
def generate_top_summary(data,snapshot_index,ports_num,CONVER_PATH):
    ...
    data_acc['fulfillment_ratio'] = list(
        map(lambda x, y: float('{:.4f}'.format(x/(y+1/1000))), data_acc['acc_fulfillment'], data_acc['acc_booking']))
    ...
    df_fulfillment = data_acc[['port name','acc_fulfillment']].sort_values(by='acc_fulfillment',ascending=False).head(5)
...
def generate_detail_plot_by_snapshot(info_selector, data, snapshot_index, ports_num,CONVER_PATH,sample_ratio_res,item_option=None):
...
def generate_detail_plot_by_ports(info_selector, data, str_temp, snapshot_num,snapshot_sample_num,item_option=None):
    ...
    bar_data_long_form = bar_data.melt('snapshot_index', var_name='Attributes',
                                       value_name='count')
    ...
    if senario == 'CIM':
        ...
        if option_2 == 'by snapshot':
            ...
            item_option = st.multiselect(
                ' ', item_option_all, item_option_all)
            item_option = get_CIM_item_option(item_option, item_option_all)
            generate_detail_plot_by_snapshot(specific_info, data_ports, snapshot_index, ports_num, CONVER_PATH,sample_ratio_res,
                                             item_option)
        ...
        snapshots_index = np.arange(snapshot_num).tolist()
        item_option_all = ['All', 'Requirement Info', 'Station Info', 'bikes', 'shortage', 'trip_requirement', 'fulfillment', 'capacity','extra_cost','failed_return']
...
def formula_define(data_origin):
    ...
    if formula_select=='a+b':
        ...
        if len(res)==0 or res[0]=="":
            ...
            if data_right:
                data_origin[res[0]+'+'+res[1]]=list(map(lambda x,y: x+y, data_origin[res[0]], data_origin[res[1]]))
            else:
                return
        ...
        if len(res) == 0 or res[0] == "":
            ...
            if data_right:
                data_origin[res[0]+'*'+res[1]+'+sqrt('+res[2]+'*+'+res[3]+')'] = list(
                    map(lambda x, y,z,w: int(x)*int(y)+math.sqrt((z)*int(w)), data_origin[res[0]], data_origin[res[1]],data_origin[res[2]],data_origin[res[3]]))
            else:
                return
...
def show_summary_plot(senario, ROOT_PATH, CONVER_PATH,ports_file_path, stations_file_path):
    ...
    if senario == 'CIM':
        ...
        df_requirement = data[['port name', 'trip_requirement']].sort_values(by='trip_requirement', ascending=False).head(5)
        df_fulfillment = data[['port name', 'fulfillment']].sort_values(by='fulfillment', ascending=False).head(5)
        df_fulfillment_ratio = data[['port name', 'fulfillment_ratio']].sort_values(by='fulfillment_ratio', ascending=False).head(5)
