...
angular.module('grafana.directives').directive('dotchartLegend', (popoverSrv: any, $timeout: any) => {
  return {
    link: (scope: any, elem: any) => {
      const $container = $('<div class="dotchart-legend__container"></div>');
      ...
      function render() {
        if (panel.legendType === 'On graph' || !panel.legend.show) {
          $container.empty();
          elem.find('.dotchart-legend').css('padding-top', 0);
          return;
        } else {
          elem.find('.dotchart-legend').css('padding-top', 6);
        }

        if (firstRender) {
          elem.append($container);
          $container.on('click', '.dotchart-legend-icon', openColorSelector);
          $container.on('click', '.dotchart-legend-alias', toggleSeries);
        ...
        for (i = 0; i < seriesList.length; i++) {
          ...
          let html = '<div class="dotchart-legend-series';
          if (ctrl.hiddenSeries[seriesData.label]) {
            html += ' dotchart-legend-series-hidden';
          }
          html += '" data-series-index="' + i + '">';
          html += '<span class="dotchart-legend-icon" style="float:none;">';
          html += '<i class="fa fa-minus pointer" style="color:' + seriesData.color + '"></i>';
          html += '</span>';

          html += '<a class="dotchart-legend-alias" style="float:none;">' + _.escape(seriesData.label) + '</a>';
