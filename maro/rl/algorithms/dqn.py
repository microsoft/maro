...
class DQN(AbsAlgorithm):
    ...
    def load_models(self, model_dict):
        ...
        self._eval_model.load_state_dict(model_dict["eval"])
        self._target_model.load_state_dict(model_dict["target"])

    def dump_models(self):
        """Return the eval model."""
        return {"eval": self._eval_model.state_dict(), "target": self._target_model.state_dict()}

    def load_models_from_file(self, path):
        """Load the eval model from disk."""
        self.load_models(torch.load(path))
