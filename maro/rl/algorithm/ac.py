...
from maro.rl.model import ParameterizedPolicyWithValueEstimator
...
class ActorCritic(AbsAlgorithm):
    ...
    Args:
        model (ParameterizedPolicyWithValueEstimator): Multi-task model that computes action distributions and
            state values. It may or may not have a shared bottom stack.
    ...
    def __init__(
        self,
        model: ParameterizedPolicyWithValueEstimator,
        ...
        if not isinstance(model, ParameterizedPolicyWithValueEstimator):
            raise TypeError("model must be an instance of 'ParameterizedPolicyWithValueEstimator'")
    ...
    def choose_action(self, state: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        with torch.no_grad():
            actions, log_p = self.model.choose_action()
        actions, log_p = actions.cpu().numpy(), log_p.cpu().numpy()
        if len(actions) == 1:
            return actions[0], log_p[0]
        else:
    ...
    def step(self):
        ...
        state_values = self.model(states, output_action_probs=False).detach().squeeze()
        next_state_values = self.model(next_states, output_action_probs=False).detach().squeeze()
        return_est = rewards + self.config.reward_discount * next_state_values
        advantages = return_est - state_values
        for i in range(self.config.train_iters):
            # actor loss
            action_prob, state_values = self.model(states)
