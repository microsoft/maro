...
class EventPool:
    ...
    def gen(self, tick: int, event_type: object, payload: object, is_cascade: bool = False) -> ActualEvent:
        """Generate an event.
        ...
        event.reset_value(
            id=next(self._event_count), tick=tick, event_type=event_type, payload=payload, state=EventState.PENDING
        )
        return event
