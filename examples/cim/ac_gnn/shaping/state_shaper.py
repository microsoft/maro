...
class StateShaper:
    ...
    Args:
        ...
        attention_order (str): "temporal" or "random". If "temporal", the edges in the graph are ordered according to
            event time. Otherwise, they are ordered randomly. 
        onehot_identity (bool): If True, one-hot identity will be used. Defaults to False.
    ...
    def __init__(
            self, port_code_list, vessel_code_list, max_tick, attention_order="temporal", onehot_identity=False, 
        ...
        self.onehot_identity = onehot_identity
    ...
    def sort(self, arrival_time, attr=None):
        ...
        if self.attention_order == "ramdom":
    ...
    def get_input_dim(self, agent_code):
        if agent_code in self.port_code_inv_dict or agent_code == "p":
            return len(self.port_features) + (self.node_cnt if self.onehot_identity else 0)
        elif agent_code in self.vessel_code_inv_dict or agent_code == "v":
            return len(self.vessel_features) + (self.node_cnt if self.onehot_identity else 0)
