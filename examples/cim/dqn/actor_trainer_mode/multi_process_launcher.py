...
if __name__ == "__main__":
    ...
    trainer_path = f"{os.path.split(os.path.realpath(__file__))[0]}/trainer.py &"
    actor_path = f"{os.path.split(os.path.realpath(__file__))[0]}/actor.py &"

    # Launch the learner process
    os.system(f"GROUP={args.group_name} NUM_ACTORS={args.num_actors} MODE={args.mode} python " + trainer_path)
