...
def set_input_dim(config):
    ...
    look_back = config["env"]["state_shaping"]["look_back"]
    max_ports_downstream = config["env"]["state_shaping"]["max_ports_downstream"]
    num_port_attributes = len(config["env"]["state_shaping"]["port_attributes"])
    num_vessel_attributes = len(config["env"]["state_shaping"]["vessel_attributes"])
