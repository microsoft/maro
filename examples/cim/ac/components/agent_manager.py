...
class ACAgentManager(AbsAgentManager):
    def _assemble(self, agent_dict):
        ...
        for agent_id in self._agent_id_list:
            ...
            value_model = LearningModel(decision_layers=MLPDecisionLayers(name=f'{agent_id}.policy',
                                                                          input_dim=self._state_shaper.dim,
                                                                          output_dim=num_actions,
                                                                          **config.agents.algorithm.value_model))

            algorithm = ActorCritic(policy_model=policy_model,
                                    ...
                                    hyper_params=ActorCriticHyperParameters(num_actions=num_actions,
