...
from .config import algorithm, action_shaping_conf, device_mapping, num_agents, state_dim
...
if algorithm == "ac":
    ...
    trainer_creator = {f"ac_{i}": partial(get_ac, state_dim, device_mapping[i]) for i in range(num_agents)}
elif algorithm == "ppo":
    ...
    trainer_creator = {f"ppo_{i}": partial(get_ppo, state_dim, device_mapping[i]) for i in range(num_agents)}
elif algorithm == "dqn":
    ...
    trainer_creator = {f"dqn_{i}": partial(get_dqn, device_mapping[i]) for i in range(num_agents)}
