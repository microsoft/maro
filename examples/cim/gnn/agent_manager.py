...
class SimpleAgentManger(AbsAgentManager):
    def __init__(self, name, agent_id_list, ort_code_list, vessel_code_list, demo_env, state_shaper: GNNStateShaper,
            logger=DummyLogger()):
        super().__init__(name, AgentMode.TRAIN, agent_id_list, state_shaper=state_shaper, action_shaper=None,
    ...
    def assemble(self, config):
        ...
        p_dim, pedge_dim, p_cnt = self._state_shaper.get_input_dim("p"), self._state_shaper.get_input_dim("pedge"),
                                        len(self.port_code_list)

        self.device = torch.device(config.training.device)
        self._logger.info(config.training.device)
        ac_model = SharedAC(p_dim, pedge_dim, v_dim, vedge_dim, config.model.tick_buffer, config.model.action_dim). \
                            to(self.device)

        value_dict = {
            ("s", "v"): ((config.model.tick_buffer, len(self.vessel_code_list), self._state_shaper.get_input_dim("v")),
                np.float32, False),
            ("s", "p"): ((config.model.tick_buffer, len(self.port_code_list), self._state_shaper.get_input_dim("p")),
                np.float32, False),
            ...
            ("s", "vedge"): ((len(self.vessel_code_list), len(self.port_code_list),
                self._state_shaper.get_input_dim("vedge")), np.float32, True),
            ("s", "pedge"): ((len(self.port_code_list), len(self.vessel_code_list),
                self._state_shaper.get_input_dim("vedge")), np.float32, True),
            ("s", "ppedge"): ((len(self.port_code_list), len(self.port_code_list),
                self._state_shaper.get_input_dim("pedge")), np.float32, True),
            ("s", "mask"): ((config.model.tick_buffer, ), np.bool, True),

            ("s_", "v"): ((config.model.tick_buffer, len(self.vessel_code_list), self._state_shaper.get_input_dim("v")),
                np.float32, False),
            ("s_", "p"): ((config.model.tick_buffer, len(self.port_code_list), self._state_shaper.get_input_dim("p")),
                np.float32, False),
            ...
            ("s_", "vedge"): ((len(self.vessel_code_list), len(self.port_code_list),
                self._state_shaper.get_input_dim("vedge")), np.float32, True),
            ("s_", "pedge"): ((len(self.port_code_list), len(self.vessel_code_list),
                self._state_shaper.get_input_dim("vedge")), np.float32, True),
            ("s_", "ppedge"): ((len(self.port_code_list), len(self.port_code_list),
                self._state_shaper.get_input_dim("pedge")), np.float32, True),
