...
def create_po_agents(config):
    set_seeds(config.seed)
    agent_dict = {}
    for agent_id in config.names:
        ...
        if config.type == "actor_critic":
            ...
            agent_dict[agent_id] = ActorCritic(
                model, ActorCriticConfig(critic_loss_func=nn.SmoothL1Loss(), **hyper_params)
            )
        else:
            ...
            agent_dict[agent_id] = PolicyGradient(model, config.reward_discount)
