...
TARGET_UPDATE_FREQ = config.train.reinforce.target_update_frequency
LEARNING_RATE = config.train.reinforce.lr
DROPOUT_P = config.train.reinforce.dropout_p
...
reinforce_LOG_ENABLE = config.log.reinforce.enable
reinforce_LOG_DROPOUT_P = config.log.reinforce.dropout_p
ACTORNET_LOG_ENABLE = config.log.actornet.enable
ACTOR_SEED = config.actornet.seed
SYNC_TRAINING = config.train.sync_training
...
class Runner:
    ...
    def _load_agent(self, agent_idx_list: [int]):
        ...
        for agent_idx in agent_idx_list:
            ...
            policy_net = ActorNet(name=f'{self._port_idx2name[agent_idx]}.policy', input_dim=state_shaping.dim,
                              hidden_dims=[256, 128, 64], output_dim=len(action_space),
                              log_enable=ACTORNET_LOG_ENABLE, log_folder=LOG_FOLDER)
            reinforce = Reinforce(policy_net=policy_net, lr=LEARNING_RATE,
                      log_enable=reinforce_LOG_ENABLE, log_folder=LOG_FOLDER, log_dropout_p=reinforce_LOG_DROPOUT_P)
            agent_dict[agent_idx] = Agent(agent_name=self._port_idx2name[agent_idx], port_idx2name=self._port_idx2name,
                                          vessel_idx2name=self._vessel_idx2name,
                                          algorithm=reinforce, experience_pool=experience_pool,
