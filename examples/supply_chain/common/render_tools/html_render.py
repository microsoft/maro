        ...
        for facility_name in facility_list:
            ...
            for sku_name in sku_list:
                ...
                if df.shape[0] > 0:
                    ...
                    y_product_cost = [
                        round(x, 0)
                        for x in (df["consumer_order_product_cost"] + df["consumer_order_base_cost"]).tolist()
                    ]
                    y_oos_loss = [
                        round(x, 0)
                        for x in (
                    ...
                    y_distribution_cost = [
                        round(x, 0)
                        for x in (df["product_transportation_cost"] + df["product_delay_order_penalty"]).tolist()
                    ]
