...
class Dashboardciti_bike(DashboardBase):
    # info dictionary
    static_info = {}
    dynamic_info = {}
    ranklist_info = {}
    ...
    def __init__(self, experiment: str, log_folder: str, host: str = 'localhost', port: int = 50301, use_udp: bool = True, udp_port: int = 50304):
        ...
        Dashboardciti_bike.static_info['name'] = experiment
        psutil.cpu_percent()
    ...
    def upload_exp_data(self, fields: dict, ep: int, tick: int, measurement: str) -> None:
        ...
        """
        tag = {}
        if 'COMPONENT_NAME' in Dashboardciti_bike.static_info:
            tag['component_name'] = Dashboardciti_bike.static_info['COMPONENT_NAME']
        fields['ep'] = ep
        if tick is not None:
            fields['tick'] = tick
    ...
    def update_static_info(self, info:dict) -> None:
        for info_name, info_value in info.items():
            Dashboardciti_bike.static_info[info_name] = info_value
        
    def update_dynamic_info(self, info:dict) -> None:
        for info_name, info_value in info.items():
            Dashboardciti_bike.dynamic_info[info_name] = info_value
        Dashboardciti_bike.dynamic_info['cpu_percent'] = psutil.cpu_percent()
        memory_usage = dict(psutil.virtual_memory()._asdict())
        for memory_usage_name, memory_usage_value in memory_usage.items():
            Dashboardciti_bike.dynamic_info[f'memory_{memory_usage_name}'] = memory_usage_value

    def update_ranklist_info(self, info:dict) -> None:
        for info_name, info_value in info.items():
            Dashboardciti_bike.ranklist_info[info_name] = info_value

    def upload_to_ranklist(self, ranklist: str, fields: dict) -> None:
        for info_name, info_value in Dashboardciti_bike.ranklist_info.items():
            fields[info_name] = info_value
        super(Dashboardciti_bike, self).upload_to_ranklist(ranklist=ranklist, fields=fields)
