...
class CitibikeStateShaping(BaseStateShaping):
    ...
    def get_station_features(self, cur_frame_index: int, env_tick: int):
        ...
        # add hour, week, month
        temporal_feature = np.concatenate((ticks//1440%30, ticks//1440%7, ticks%1440//60), axis=1).reshape(self._td_steps, 1, 3)
        temporal_feature = np.repeat(temporal_feature, self._station_cnt, axis=1).reshape(self._td_steps, self._station_cnt, 3)
        # change to (station_cnt, tick_len, attribute_len)
        station_features = np.concatenate((raw_features.transpose((2, 0, 1)), temporal_feature.transpose(1, 0, 2)), axis=-1)
