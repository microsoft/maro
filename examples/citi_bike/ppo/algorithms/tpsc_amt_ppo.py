...
import numpy as np
import os
from examples.citi_bike.ppo.models.homo_gnn import GNNBackend, Policy, AttAmtPolicy, AttPolicy, STGNNBackend
...
class AttGnnPPO:
    def __init__(self, node_dim, channel_cnt, graph_size, log_pth, device='cuda:0', **kargs):
        ...
        self.temporal_gnn = GNNBackend(time_window=20, node_dim=node_dim,output_dim=self.emb_dim,output_channels=self.channel_cnt)
    ...
    def act(self, obs):
        ...
        '''
        with torch.no_grad():
            x, time_feature, edge_idx_list, action_edge_idx, actual_amount, per_graph_size = time_obs_to_torch(obs, self.device)
            x = x.transpose(0,1)
    ...
    def grad(self, batch):
        ...
        # print("gamma",gamma)
        print(batch['s'])
        x, time_feature, edge_idx_list, action_edge_idx, actual_amount, per_graph_size = time_obs_to_torch(batch['s'], self.device)
        x_, time_feature_, edge_idx_list_, action_edge_idx_, actual_amount_, per_graph_size_ = time_obs_to_torch(batch['s_'], self.device)
