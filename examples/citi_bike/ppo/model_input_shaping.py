...
class StateShaping:
    ...
    def __call__(self, decision_event):
        ...
        trip_raw = self._env.snapshot_list.matrix[ticks: 'trip_adj'].reshape(self._td_steps, self._station_cnt, self._station_cnt)
        trip_adj = np.transpose(trip_raw, (1,2,0))
        distance_mat = self._env.snapshot_list.matrix[0:'distance_adj'].reshape(self._station_cnt, self._station_cnt, 1)*self.distance_scaler
        ...
        if decision_event.type == DecisionType.Supply:
            actual_amount = np.ones(len(decision_event.action_scope))*self._env.snapshot_list.static_nodes[cur_tick:decision_event.cell_idx:(['bikes'], 0)][0]
            actual_amount = np.hstack([actual_amount.reshape(-1,1), np.zeros((*actual_amount.shape,1))])
        else:
            ftmp, btmp = self._env.snapshot_list.static_nodes[cur_tick:decision_event.cell_idx:(['capacity', 'bikes'], 0)]
