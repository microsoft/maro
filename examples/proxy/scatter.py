...
def summation_worker(group_name):
    ...
    """
    proxy = Proxy(group_name=group_name, component_type="sum_worker", expected_peers={"master": 1})
...
def master(group_name: str, sum_worker_number: int, multiply_worker_number: int, is_immediate: bool = False):
    ...
    proxy = Proxy(
        ...
        expected_peers={"sum_worker": sum_worker_number, "multiply_worker": multiply_worker_number},
    )
    ...
    if is_immediate:
        session_ids = proxy.iscatter(
            tag="job", session_type=SessionType.TASK, destination_payload_list=destination_payload_list
        )
        # Do some tasks with higher priority here.
        replied_msgs = proxy.receive_by_id(session_ids, timeout=-1)
    else:
        replied_msgs = proxy.scatter(
            tag="job", session_type=SessionType.TASK, destination_payload_list=destination_payload_list, timeout=-1
        )
